    ...
    <div v-if="messages" class="messages">
      <ul v-for="message in messages" :key="message.id">
        <li :class="{ received: message.email !== uid, sent: message.email === uid }">
          ...
          <div class="message" @dblclick="createLike(message.id)" @click.right="deleteLike(message.id)">
            {{ message.content }}
...
export default {
  ...
  methods: {
    ...
    async deleteLike(messageId) {
      try {
        const res = await axios.delete(`http://localhost:3000/likes/${messageId}`,
          {
            headers: {
              uid: this.uid,
              "access-token": window.localStorage.getItem('access-token'),
